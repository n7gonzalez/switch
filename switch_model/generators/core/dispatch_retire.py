...
def define_components(mod):
        ...
        if g not in m.CCS_EQUIPPED_GENS:
            ...
            return (
                ...
                #(m.f_co2_intensity[f] * ccs_emission_frac +
                 ...
                 (m.co2_load_zone_intensity[g]* ccs_emission_frac+m.f_upstream_co2_intensity[f]))
    '''
    def DispatchEmissions_rule(m, g, t, f):
        return (m.GenFuelUseRate[g, t, f] *(m.CO2_load_zone_intensity[g]))
    ...
    '''
    def NOx_DispatchEmissions_rule(m, g, t, f):
        return (m.GenFuelUseRate[g, t, f] *(m.SO2_load_zone_intensity[g]))
    def SO2_DispatchEmissions_rule(m, g, t, f):
        return (m.GenFuelUseRate[g, t, f] *(m.NOx_load_zone_intensity[g]))
    def PM25_DispatchEmissions_rule(m, g, t, f):
        return (m.GenFuelUseRate[g, t, f] *(m.PM25_load_zone_intensity[g]))
    def PM10_DispatchEmissions_rule(m, g, t, f):
        return (m.GenFuelUseRate[g, t, f] *(m.PM10_load_zone_intensity[g]))
    def CO_DispatchEmissions_rule(m, g, t, f):
        return (m.GenFuelUseRate[g, t, f] *(m.CO_load_zone_intensity[g]))
    def PMcoarse_DispatchEmissions_rule(m, g, t, f):
        return (m.GenFuelUseRate[g, t, f] *(m.PMcoarse_load_zone_intensity[g]))
    def VOC_DispatchEmissions_rule(m, g, t, f):
        return (m.GenFuelUseRate[g, t, f] *(m.VOC_load_zone_intensity[g]))
    ...
    mod.PM10_DispatchEmissions = Expression(
        mod.GEN_TP_FUELS,
        rule=PM10_DispatchEmissions_rule)
    mod.PM10_AnnualEmissions = Expression(mod.PERIODS,
        rule=lambda m, period: sum(
            m.PM10_DispatchEmissions[g, t, f] * m.tp_weight_in_year[t]
            for (g, t, f) in m.GEN_TP_FUELS
            if m.tp_period[t] == period),
        doc="The system's annual emissions, in metric tonnes of PM25 per year.")
    mod.PMcoarse_DispatchEmissions = Expression(
        mod.GEN_TP_FUELS,
        rule=PMcoarse_DispatchEmissions_rule)
    mod.PMcoarse_AnnualEmissions = Expression(mod.PERIODS,
        rule=lambda m, period: sum(
            m.PMcoarse_DispatchEmissions[g, t, f] * m.tp_weight_in_year[t]
            for (g, t, f) in m.GEN_TP_FUELS
            if m.tp_period[t] == period),
        doc="The system's annual emissions, in metric tonnes of PM25 per year.")

    mod.CO_DispatchEmissions = Expression(
        mod.GEN_TP_FUELS,
        rule=CO_DispatchEmissions_rule)
    mod.CO_AnnualEmissions = Expression(mod.PERIODS,
        rule=lambda m, period: sum(
            m.CO_DispatchEmissions[g, t, f] * m.tp_weight_in_year[t]
            for (g, t, f) in m.GEN_TP_FUELS
            if m.tp_period[t] == period),
        doc="The system's annual emissions, in metric tonnes of PM25 per year.")
        
    mod.VOC_DispatchEmissions = Expression(
        mod.GEN_TP_FUELS,
        rule=VOC_DispatchEmissions_rule)
    mod.VOC_AnnualEmissions = Expression(mod.PERIODS,
        rule=lambda m, period: sum(
            m.VOC_DispatchEmissions[g, t, f] * m.tp_weight_in_year[t]
            for (g, t, f) in m.GEN_TP_FUELS
            if m.tp_period[t] == period),
        doc="The system's annual emissions, in metric tonnes of PM25 per year.")
...
def post_solve(instance, outdir):
    ...
    dispatch_normalized_dat = [{
        ...
        "DispatchEmissions_tSO2_per_typical_yr": value(sum(
            instance.SO2_DispatchEmissions[g, t, f] * instance.tp_weight_in_year[t]
              for f in instance.FUELS_FOR_GEN[g]))if instance.gen_uses_fuel[g] else None,

        "DispatchEmissions_tNOx_per_typical_yr": value(sum(
            instance.NOx_DispatchEmissions[g, t, f] * instance.tp_weight_in_year[t]
              for f in instance.FUELS_FOR_GEN[g]))if instance.gen_uses_fuel[g] else None,

        "DispatchEmissions_tPM25_per_typical_yr": value(sum(
            instance.PM25_DispatchEmissions[g, t, f] * instance.tp_weight_in_year[t]
              for f in instance.FUELS_FOR_GEN[g]))if instance.gen_uses_fuel[g] else None,

        "DispatchEmissions_tPM10_per_typical_yr": value(sum(
            instance.PM10_DispatchEmissions[g, t, f] * instance.tp_weight_in_year[t]
              for f in instance.FUELS_FOR_GEN[g]))if instance.gen_uses_fuel[g] else None,              
        "DispatchEmissions_tPMcoarse_per_typical_yr": value(sum(
            instance.PMcoarse_DispatchEmissions[g, t, f] * instance.tp_weight_in_year[t]
              for f in instance.FUELS_FOR_GEN[g]))if instance.gen_uses_fuel[g] else None,
        "DispatchEmissions_tCO_per_typical_yr": value(sum(
            instance.CO_DispatchEmissions[g, t, f] * instance.tp_weight_in_year[t]
              for f in instance.FUELS_FOR_GEN[g]))if instance.gen_uses_fuel[g] else None,
        "DispatchEmissions_tVOC_per_typical_yr": value(sum(
            instance.VOC_DispatchEmissions[g, t, f] * instance.tp_weight_in_year[t]
              for f in instance.FUELS_FOR_GEN[g]))if instance.gen_uses_fuel[g] else None
    ...
    annual_summary.to_csv(
        ...
        columns=["Energy_GWh_typical_yr", "VariableCost_per_yr", 
                #"DispatchEmissions_tCO2_per_typical_yr"])
                 ...
                 "DispatchEmissions_tPM25_per_typical_yr",
                 "DispatchEmissions_tPM10_per_typical_yr",
                 "DispatchEmissions_tPMcoarse_per_typical_yr",
                 "DispatchEmissions_tCO_per_typical_yr",
                 "DispatchEmissions_tVOC_per_typical_yr"])
