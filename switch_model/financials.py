...
def define_dynamic_components(mod):
    ...
    def System_Cost_rule(m):
        ...
        if 'HYDRO_GEN_TS' in dir(m):
            system_cost += SLACK_VARIABLE_PENALTY*100000000*sum(m.HydroEnfroceMinSlack[gt] for gt in m.HYDRO_GEN_TPS)+SLACK_VARIABLE_PENALTY*100000000*sum(m.HydroEnfroceMaxSlackPositive[gt]+m.HydroEnfroceMaxSlackNegative[gt] for gt in m.HYDRO_GEN_TPS)
        return system_cost
