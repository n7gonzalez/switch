...
def define_dynamic_components(mod):
    ...
    mod.SystemCost = Expression(
        rule=lambda m: sum(m.SystemCostPerPeriod[p] for p in m.PERIODS) + 1000000000000*(sum(m.LoadZonePositive[zt] + m.LoadZoneNegative[zt] for zt in m.ZONE_TIMEPOINTS) + sum(m.MaxBuildPotentialSlack[gp] for gp in m.CAPACITY_LIMITED_GENS * m.PERIODS) + sum(m.RPSEnforceTargetSlack[p] for p in m.RPS_PERIODS)))
